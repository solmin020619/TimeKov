...
public class CategoryFilterUI : MonoBehaviour
{
    ...
    private void ApplyLayoutInstant(int sel)
    {
        ...
        for (int i = 0; i < n; i++) widths[i] = (i == sel) ? _expandedWidth[i] : collapsedWidth;
        LayoutWidths(widths);
        for (int i = 0; i < n; i++)
            if (_tabLabels[i] != null)
            {
                var c = _tabLabels[i].color; c.a = (i == sel) ? 1f : 0f; _tabLabels[i].color = c;
            }
    }
    ...
    {
        if (_tabRects == null) return;
        if (!isActiveAndEnabled) { ApplyLayoutInstant(sel); return; }
