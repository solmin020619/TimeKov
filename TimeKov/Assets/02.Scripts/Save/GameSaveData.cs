...
public class GameSaveData
{
    ...
    // ── 기믹/퍼즐/상자 진행 상태 (GimmickSave) ──────────────────────────────
    // 스위치 On/Off · 에너지 노드에 넣은 연료 · 파괴물 타격 수 · 순서 퍼즐 진행도 · 조건 충족
    // 여부 · 상자의 리젠 남은 시간까지, "다음에 들어왔을 때 그대로여야 하는" 것을 전부
    // id → 값 하나로 여기에 모은다.
    //   ★값을 float 하나로 통일한 이유: 종류마다 타입이 달라도(bool/개수/진행도/남은 초)
    //     저장 형태가 같으면 새 기믹을 추가할 때 세이브 구조를 안 건드려도 된다.
    //   ★id 앞에 종류 접두어가 붙어(sw / node / brk / seq / trg.* / chest.*) 서로 안 겹친다.
