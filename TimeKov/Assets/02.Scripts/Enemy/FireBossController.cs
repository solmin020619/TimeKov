...
public class FireBossController : MonoBehaviour, IEnemyDataSource
{
    ...
    private void Update()
    {
        ...
        {
            _engaged = true;
            BossHealthBarUI.Show(_health, data != null ? Loc.Get(data.enemyName) : Loc.Get("화염정령"), Loc.Get(bossSubtitle));
            _feedback?.PlayDetect();
            BattleBgm.Begin(SfxId.WyvernBattleBgm);   // 교전 시작 → 전투 브금(기존 BGM 일시정지). 보스 공통 브금 재사용.
        }
        if (target == null) { TickDisengage(); return; }   // 고정형 = 추격 없음. 애니는 exitTime 으로 알아서 Idle 복귀(매프레임 crossfade 금지)
