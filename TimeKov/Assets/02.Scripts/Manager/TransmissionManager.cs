...
public class TransmissionManager : MonoBehaviour, ISaveable
{
    ...
    //   5=시간에너지합성기(3)+용해로(6), 10=창고출력포트(9), 15=저장고(8),
    //   20=생체분리기(4)+에너지변환기(5), 30=코어합성기(7).
    //
    // [순환 데드락 해소] 용해로(6)는 반드시 합성기(3)와 같은 5% 에 있어야 한다.
    //   충전 키트 = 빈 시간키트(설비3) + 조각 / 빈 시간키트 = 금속 주괴 x2 / 금속 주괴 = 용해로(6).
    //   용해로가 10% 에 있으면 "10% 를 채우려면 용해로가 필요하고 용해로는 10% 에서 열린다" 가 되어
    //   스타터 키트로 5% 를 찍은 뒤 영구히 막힌다(에디터는 백쿼트 전체해금 때문에 안 걸림 = 정식 빌드에서만 터짐).
    //
    // [데모] 자연맵 전용 빌드는 전송률 상한이 25% 라 그 안에 설비를 몰아넣었다.
    //   생체분리기(4)+에너지변환기(5)를 60 -> 20 으로 당긴 이유 = 배양체 루프(원료 2배)가
    //   자연 원료(거미독액/점액/눈알/부식액)만으로 돌아서 데모에서 실제로 쓸 수 있기 때문.
    //   코어합성기(7)는 30 에 그대로 둔다 - 코어 키트 재료(회로판/구리선)가 다른 맵이라 열어도 못 쓴다.
    //   본편 복귀 시 20 -> 60, 10/15 를 15/20 으로 되돌리면 된다.
    // [연마기(10)] 08-13 추가. 그전까지 이 표에 없어서 정식 플레이로는 영영 안 열렸다
    //   (에디터는 백쿼트 전체해금 때문에 안 걸려서 못 보고 지나갔다 - 8번과 같은 유형의 사고).
    //   15% 인 이유: 만드는 것이 화학 촉매(생체 배양액 -> 중급 회복 앰플) / 광학 렌즈(코어 결정) /
    //   과열 촉진액(용암)이라 전부 설원 진입(25%) 이후에 쓰인다. 그 직전에 열어 준비시킨다.
    private static int[] MilestoneFacilityIds(int pct) => pct switch
    {
        ...
        15 => new[] { 8, 10 },
        30 => new[] { 7 },
        // 생체 분리기(4) + 에너지 변환기(5) = 원재료 무한복제 쌍(4->5->4 로 원재료가 순증한다).
        // 후반 보상으로 설계된 물건인데 예전엔 20% 라 자연맵 중반에 손에 들어왔고, 그때부터
        // 사냥할 이유가 사라졌다. 후반 마일스톤으로 미룬다.
        // ★craftTime 으로 느리게 만들어 억제하지 마라 - 조작감만 답답해지고 후반 가치도 같이 죽는다.
        70 => new[] { 4, 5 },
