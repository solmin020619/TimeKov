...
public class GameUIController : MonoBehaviour
{
    ...
    public void HandleEscape()
    {
        ...
        {
            // 새 설정 UI(GameSettingsUI)가 열려 있으면 그쪽 경고창을 쓴다.
            // 구 UI의 applyWarningModal은 이제 씬에 없어서, 매니저의 RequestClose()만 믿으면
            // 경고도 안 뜨는데 닫히지도 않는 상태가 된다.
            var newPanel = GameSettingsUI.SettingsUIBuilder.Active;
            if (newPanel != null)
            {
                if (!newPanel.TryCloseFromEscape()) return;
            }
            else if (GlobalSettingsManager.Instance != null
