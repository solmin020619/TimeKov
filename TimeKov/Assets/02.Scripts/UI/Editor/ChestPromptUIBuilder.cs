...
// [08-02] 상자 프롬프트(ChestPromptUI) 실물 생성기.
//   예전엔 ChestPromptUI 가 실행 중에 계층을 통째로 만들었다. 이제 여기서 Canvas 아래 실물로 만든다.
//   스프라이트는 RuntimeRoundedSprite 가 실행 시 채운다(UISpriteFactory 생성물은 저장이 안 되므로).
public static class ChestPromptUIBuilder
{
    private const string RootName = "ChestPrompt";
    ...
    public static void Build()
    {
        ...
        if (Object.FindAnyObjectByType<ChestPromptUI>(FindObjectsInactive.Include) != null)
        {
            if (!EditorUtility.DisplayDialog("확인", "ChestPrompt 가 이미 있습니다. 지우고 다시 만들까요?", "다시 만들기", "취소")) return;
            UIBuilderUtil.RemoveExisting<ChestPromptUI>();
        }
        ...
        Undo.RegisterCreatedObjectUndo(rootGo, "Build ChestPrompt");
        ...
        // 진행 섹션
        var (progressSection, progressFill, timerText) = BuildProgressGroup(panelGo);
        ...
        // 컴포넌트 + 참조 연결
        var ui = rootGo.AddComponent<ChestPromptUI>();
        ...
        EditorUtility.DisplayDialog("완료",
            "상자 프롬프트(ChestPrompt)를 만들었습니다.\n\n" +
            "위치: Canvas/Overlays/ChestPrompt\n" +
    ...
    private static (GameObject section, Image fill, TMP_Text timer) BuildProgressGroup(GameObject parent)
    {
        ...
        AddTMP(row, "Lbl", "여는 중", 12f, SubText, TextAlignmentOptions.MidlineLeft);
    ...
    private static void SetRef(SerializedObject so, string field, Object value)
    {
        ...
        else Debug.LogWarning($"[ChestPromptUIBuilder] 필드를 못 찾음: {field}");
    }
}
