...
public static class TutorialAssetBuilder
{
    ...
    // 본편 복귀 = false 로 바꾸고 Tools/TIMEKOV/퀘스트/튜토리얼 에셋 생성 재실행.
    ...
    [MenuItem("Tools/TIMEKOV/퀘스트/튜토리얼 에셋 생성")]
    ...
    static QuestSO BuildLootVideoQuest()
        => BuildQuest("quest_tut_02v_loot_video", "전투 자원 회수 방법 확인하기",
            CreateVideoTutorial("obj_loot_video", "전투 자원 회수 방법을 확인하세요.",
                // ★'결계 안과 밖'을 맨 앞에 둔다. 이 게임에서 제일 중요한 생존 정보(밖에 있으면 시간이 준다)인데
                //   예전엔 5장 중 3번째라, 페이지를 연타로 넘기는 사람에게는 나머지와 함께 묻혔다.
                //   1장에 두면 연타하는 사람도 첫 화면에서 이건 본다.
                VPage("결계 안과 밖",
